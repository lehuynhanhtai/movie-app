...
export default function MovieByCategory({ movieByCate }: any) {
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-2xl font-bold">{movieByCate.titlePage}</h3>
        <Link href={`/the-loai/${movieByCate.type_list}`}>
          <Button
            variant="link"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3">
        {movieByCate.items.map((movie: any) => (
          <div
            key={movie._id}
