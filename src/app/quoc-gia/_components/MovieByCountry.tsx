...
export default function MovieByCountry({ movieByCountry }: any) {
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-xl font-bold">{movieByCountry.titlePage}</h3>
        <Link href={`/the-loai/${movieByCountry.type_list}`}>
          <Button
            variant="link"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3">
        {movieByCountry.items.map((movie: any) => (
          <div
            key={movie._id}
